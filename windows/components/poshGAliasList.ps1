...
$poshGitAliasList = @{
    ...
    ga   = "git add"
    gaa  = "git add --all -A"
    gapa = "git add --patch -p"
    gau  = "git add --update -u"
    gav  = "git add --verbose -v"
    # gap  = "git apply"
    # gapt = "git apply --3way"
    gb   = "git branch"
    ...
    # gbD ="git branch -D" -- Duplicate keys 'gbD' are not allowed in hash literals.
    gbl  = "git blame -b -w"
...
foreach ($key in $poshGitAliasList.Keys) {
    $poshGitAliasMap[$key] = ($poshGitAliasList[$key] -split " ")[1].Trim()
}
