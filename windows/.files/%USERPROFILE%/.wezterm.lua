...
local config = {
	...
	keys = {
		...
		{ key = "c", mods = "SHIFT|CTRL", action = act.CopyTo("Clipboard") },
		{
			key = "-",
			mods = "CTRL",
			action = act.Multiple({ act.EmitEvent("disable-unseen-temp"), act.DecreaseFontSize }),
		},
		{
			key = "=",
			mods = "CTRL",
			action = act.Multiple({ act.EmitEvent("disable-unseen-temp"), act.IncreaseFontSize }),
		},
		{
			key = "0",
			mods = "CTRL",
			action = act.Multiple({ act.EmitEvent("disable-unseen-temp"), act.ResetFontSize }),
		},
...
for _, value in ipairs(config.key_tables.wezterm) do
	table.insert(config.keys, {
		key = value.key,
		mods = value.mods,
		action = act.EmitEvent("check-nolead-" .. value.mods .. "+" .. value.key),
	})
...
local function is_in_nvim_pane(pane)
...
wezterm.on("format-tab-title", function(tab, tabs, panes, config, hover, max_width)
	...
	for _, pane in ipairs(actualPanes) do
		...
		if panePane then
			local inTmux = false
			local status, err = pcall(function()
				inTmux = panePane:get_user_vars()["TMUX"] == "1" and true or false
			end)
			if not status then
	...
	if actually_truncated then
		...
		truncated = string.sub(
			title,
			math.min(string.len(title) - recalculated_max, 1 + incrementAdjusted),
			recalculated_max + incrementAdjusted
		)
	...
	if foreground ~= nil then
		table.insert(format, { Foreground = { Color = foreground } })
	end
	if background ~= nil then
		table.insert(format, { Background = { Color = background } })
	end
